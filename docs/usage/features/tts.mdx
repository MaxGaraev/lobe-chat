---
title: GDZ.WORKS support Speech Synthesis and Recognition (TTS & STT)
description: >-
  Experience seamless Text-to-Speech (TTS) and Speech-to-Text (STT) technologies
  in GDZ.WORKS. Choose from a variety of high-quality voices for personalized
...
[`@lobehub/tts`](https://tts.gdz.works) is a high-quality TTS toolkit developed using the TS language, supporting usage in both server and browser environments.
...
<Callout type={'info'}>
  During the implementation of the TTS feature in GDZ.WORKS, we found that there was no good frontend
