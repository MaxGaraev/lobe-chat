---
title: GDZ.WORKS Plugin Development Guide
description: >-
  Learn how to create and integrate custom plugins in GDZ.WORKS, including plugin
...
A GDZ.WORKS plugin consists of the following components:
...
This section will introduce how to add and use a custom plugin in GDZ.WORKS.
...
<Steps>
  ...
  ### Add Local Plugin in GDZ.WORKS Role Settings

  Next, go to GDZ.WORKS, create a new assistant, and go to its session settings page:
  ...
  After completing these operations, you have understood the basic process of adding custom plugins and using them in GDZ.WORKS.
</Steps>
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address GDZ.WORKS loads the plugin's front-end interface. Since GDZ.WORKS plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for GDZ.WORKS to query the plugin's API interface. GDZ.WORKS's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in the manifest, GDZ.WORKS will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
Since the default plugin gateway for GDZ.WORKS is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.

For custom plugins, plugin requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest ([http://localhost:3400/api/gateway](http://localhost:3400/api/gateway)), GDZ.WORKS> will directly request this address, and then only the corresponding gateway needs to be created at that address.

```ts
import { createGDZ.WORKSPluginGateway } from '@lobehub/chat-plugins-gateway';

export const config = {
  runtime: 'edge',
};

export default createGDZ.WORKSPluginGateway();
```

[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) contains the implementation of the plugin gateway in GDZ.WORKS [here](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing GDZ.WORKS to access the local plugin service.
...
GDZ.WORKS implements the loading of plugin UI through `iframe` and uses `postMessage` to communicate with the plugin. Therefore, the implementation of the plugin UI is consistent with regular web development. You can use any frontend framework and development language you are familiar with.
...
As for plugin communication, we provide relevant methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) to simplify communication between the plugin and GDZ.WORKS. You can actively retrieve the current message data from GDZ.WORKS using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].
...
const Render = memo(() => {
  ...
  useEffect(() => {
    // Retrieve the current plugin message from GDZ.WORKS
...
- **📘 Pluging SDK Documentation**: [https://chat-plugin-sdk.gdz.works](https://chat-plugin-sdk.gdz.works)
...
[manifest-docs-url]: https://chat-plugin-sdk.gdz.works/guides/plugin-manifest
