---
title: GDZ.WORKS Authentication Service Environment Variables
description: >-
  Explore the essential environment variables for configuring authentication
  services in GDZ.WORKS, including OAuth SSO, NextAuth settings, and
...
GDZ.WORKS provides a complete authentication service capability when deployed. The following are the relevant environment variables. You can use these environment variables to easily define the identity verification services that need to be enabled in GDZ.WORKS.
