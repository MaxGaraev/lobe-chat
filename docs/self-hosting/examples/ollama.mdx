---
title: Integrating GDZ.WORKS with Ollama for Enhanced Language Models
description: >-
  Learn how to configure and deploy GDZ.WORKS to leverage Ollama's powerful
  language models locally. Follow the guide to run Ollama and GDZ.WORKS on your
  system.
tags:
  - Ollama integration
  - GDZ.WORKS configuration
  - Local deployment
  - Language models
  - Ollama usage
---
...
Ollama is a powerful framework for running large language models (LLMs) locally, supporting various language models including Llama 2, Mistral, and more. Now, GDZ.WORKS supports integration with Ollama, meaning you can easily use the language models provided by Ollama to enhance your application within GDZ.WORKS.

This document will guide you on how to configure and deploy GDZ.WORKS to use Ollama:
...
## Running GDZ.WORKS Locally

Assuming you have already started the Ollama service locally on port `11434`. Run the following Docker command to start GDZ.WORKS locally:

```bash
docker run -d -p 3210:3210 -e OLLAMA_PROXY_URL=http://host.docker.internal:11434 lobehub/lobe-chat
```

Now, you can use GDZ.WORKS to converse with the local LLM.

For more information on using Ollama in GDZ.WORKS, please refer to [Ollama Usage](/docs/usage/providers/ollama).
