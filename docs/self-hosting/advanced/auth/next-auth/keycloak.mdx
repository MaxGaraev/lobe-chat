---
title: Configuring Keycloak Authentication Service in GDZ.WORKS
description: >-
  Learn how to configure the Keycloak authentication service in GDZ.WORKS,
...
# Configuring Keycloak Authentication Service in GDZ.WORKS
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with GDZ.WORKS via Docker Compose deployment for easier service management.
</Callout>
...
- Your GDZ.WORKS database version IP/port is `http://LOBECHAT_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your GDZ.WORKS database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "GDZ.WORKS", then click "Create"
  ...
  2. Create a Client
     ...
     - On the "Login settings" page:
       ...
       - Web origins: Add your GDZ.WORKS domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the GDZ.WORKS environment variables.

  Configure the GDZ.WORKS environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/GDZ.WORKS` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/GDZ.WORKS` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/GDZ.WORKS` for Keycloak deployed in a public environment
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for GDZ.WORKS. For Keycloak, fill in `keycloak`.                                                                                                      |
...
<Callout type={'info'}>After successful deployment, users will be able to authenticate through Keycloak and use GDZ.WORKS.</Callout>
