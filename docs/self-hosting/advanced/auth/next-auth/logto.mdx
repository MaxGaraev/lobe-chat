---
title: Configuring Logto Authentication Service in GDZ.WORKS
description: >-
  Learn how to configure Logto authentication service in GDZ.WORKS, including
...
<Callout type={'tip'}>
  ...
  with the GDZ.WORKS database version. In this case, GDZ.WORKS can share the same Postgres instance
  with it.
</Callout>

## Logto Configuration Process

The following assumes your GDZ.WORKS database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  Configure the Logto Webhook so that GDZ.WORKS can receive notifications when user information is updated.
  ...
  Set the obtained `Client ID` and `Client Secret` as `AUTH_LOGTO_ID` and `AUTH_LOGTO_SECRET` in the GDZ.WORKS environment variables.

  Configure `AUTH_LOGTO_ISSUER` in the GDZ.WORKS environment variables as follows:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS`   | Required | Select the single sign-on provider for GDZ.WORKS. For Logto, enter `logto`.                                                                                                            |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate via Logto and use GDZ.WORKS.
</Callout>
