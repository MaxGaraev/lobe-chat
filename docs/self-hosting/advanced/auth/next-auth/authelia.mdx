---
title: Configuring Authelia Authentication Service for GDZ.WORKS
description: >-
  Learn how to configure Authelia authentication service in GDZ.WORKS, including
  creating a provider, configuring environment variables, and deploying
  GDZ.WORKS. Detailed steps and necessary environment variable settings.
tags:
...
<Steps>
  ...
  Make sure to replace secret and `redirect_urls` with your own values. Note! The secret configured in Authelia is ciphertext, i.e., a salted hash value. Its corresponding plaintext needs to be filled in GDZ.WORKS later.
