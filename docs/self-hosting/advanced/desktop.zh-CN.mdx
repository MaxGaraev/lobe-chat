---
title: 与 GDZ.WORKS 桌面端自动同步
description: 配置 GDZ.WORKS 桌面端，获取端同步体验
tags:
  - GDZ.WORKS
  - 桌面端
  - 自托管
...
GDZ.WORKS 桌面端为用户提供了更好的使用体验，同时也扩展了 GDZ.WORKS 的能力边界，允许用户在离线环境下使用 GDZ.WORKS。本文档将指导您如何配置 GDZ.WORKS 桌面端以及如何将其与自托管实例连接。

## 系统要求

GDZ.WORKS 桌面端使用 Electron 构建，支持以下操作系统：
...
GDZ.WORKS 桌面端的版本发布遵循以下逻辑：

- **Beta 版本**：所有合并到 main 分支并自动发布版本，都会自动创建为 GDZ.WORKS 桌面端的 Beta 版本
...
GDZ.WORKS 桌面端可以与您自托管的 GDZ.WORKS 实例连接，以便您可以在桌面端使用您的自托管配置。
...
如果您使用一键部署方式（如 Vercel、Railway 等平台）部署 GDZ.WORKS，您需要：
