---
title: GDZ.WORKS Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for GDZ.WORKS Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest GDZ.WORKS image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running GDZ.WORKS Container

  Assuming the GDZ.WORKS container is named `lobe-chat`, use the following commands to stop and remove the currently running GDZ.WORKS container:
  ...
  ### Pull the Latest GDZ.WORKS Image

  Use the following command to pull the latest Docker image for GDZ.WORKS:
  ...
  Redeploy the GDZ.WORKS container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of GDZ.WORKS's chat records are stored in your local browser. Therefore, when redeploying GDZ.WORKS using Docker, your chat records will not be lost.
</Callout>
