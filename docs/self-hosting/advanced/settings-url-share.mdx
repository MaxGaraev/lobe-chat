...
description: >-
  Learn how to import and export configuration settings for GDZ.WORKS via URL.
...
GDZ.WORKS support import settings from external URL to quickly set up GDZ.WORKS configuration.
...
```plaintext
https://gdz.works/?settings=<settings object in JSON format>
https://gdz.works/?settings={"keyVaults":{"openai":{"apiKey":"user-key","baseURL":"https://your-proxy.com/v1"}}}
```
...
<Callout type={'warning'}>
  GDZ.WORKS does not verify the correctness of the settings parameters in the URL, nor provide
