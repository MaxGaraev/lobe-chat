---
title: Deploy GDZ.WORKS with Docker
description: >-
  Learn how to deploy the GDZ.WORKS service using Docker, including installation
...
We provide a [Docker image][docker-release-link] for you to deploy the GDZ.WORKS service on your private device.
...
<Steps>
  ...
  Use the following command to start the GDZ.WORKS service with one click:
  ...
  - For the complete list of environment variables supported by GDZ.WORKS, please refer to the [Environment Variables](/docs/self-hosting/environment-variables) section.
