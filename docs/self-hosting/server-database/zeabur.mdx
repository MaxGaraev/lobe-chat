---
title: Deploying GDZ.WORKS Database on Zeabur
description: >-
  Learn how to deploy GDZ.WORKS on Zeabur with ease. Follow the provided steps to
...
# Deploying GDZ.WORKS Database on Zeabur

<Callout type="info">
  This article assumes that you are familiar with the basic principles and processes of deploying
  the GDZ.WORKS server database version, so it only includes content related to core environment
  variable configuration. If you are not familiar with the deployment principles of the GDZ.WORKS
...
- GDZ.WORKS database version.

## Deploying on Zeabur

Here is the process for deploying the GDZ.WORKS server database version on Zeabur:

<Steps>
  ### Go to the template page on Zeabur

  Go to the [GDZ.WORKS Database template page](https://zeabur.com/templates/RRSPSD) on Zeabur and click on the "Deploy" button.
  ...
  - GDZ.WORKS Domain: A free subdomain with `.zeabur.app` suffix.
  ...
  After you fill all the required environment variables, select a region where you want to deploy your GDZ.WORKS Database and click on the "Deploy" button.
  ...
  Fill in those variables into your GDZ.WORKS service on Zeabur, here is a more detailed guide for [editing environment variables on Zeabur](https://zeabur.com/docs/deploy/variables).

  For detailed configuration of Logto, refer to [this document](/docs/self-hosting/advanced/auth/next-auth/logto).

  ### Access your GDZ.WORKS Instance

  Press on the `GDZ.WORKS-Database` and you can see the public domain you just created, click on it to access your GDZ.WORKS Database.
