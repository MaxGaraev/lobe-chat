---
title: 使用 Docker 部署 GDZ.WORKS 数据库
description: 详细步骤教你如何在 Docker 中部署 GDZ.WORKS 服务端数据库。
tags:
  - Docker
  - GDZ.WORKS
  - 数据库部署
...
<Callout type="info">
  本文已经假定你了解了 GDZ.WORKS 服务端数据库版本（下简称 DB
  版）的部署基本原理和流程，因此只包含核心环境变量配置的内容。如果你还不了解 GDZ.WORKS DB
...
<Callout type="warning">
  ...
  部署 GDZ.WORKS 时，你不能使用 Clerk 作为登录鉴权服务。
...
以下是在 Linux 服务器上部署 GDZ.WORKS DB 版的流程：
...
<Steps>
  ...
  <Callout type="info">
    pgvector 插件为 Postgres 提供了向量搜索的能力，是 GDZ.WORKS 实现 RAG 的重要构件之一。
  </Callout>
...
GDZ.WORKS 的 DB 版也支持直接在本地的 Mac/Windows 本地使用。
