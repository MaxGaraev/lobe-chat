---
title: Deploying GDZ.WORKS Database with Docker
description: >-
  Learn how to deploy the GDZ.WORKS server database version using Docker on Linux
  and local machines.
tags:
  - GDZ.WORKS
  - Docker
...
<Callout type="warning">
  Due to the inability to expose `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` using Docker environment variables, you cannot use Clerk as an authentication service when deploying GDZ.WORKS using Docker / Docker Compose.
...
Here is the process for deploying the GDZ.WORKS server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    component for GDZ.WORKS to implement RAG.
  </Callout>
...
The data version of GDZ.WORKS also supports direct use on a local Mac/Windows machine.
