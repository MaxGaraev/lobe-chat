---
title: Deploying GDZ.WORKS with Docker Compose
description: >-
  Learn how to deploy the GDZ.WORKS service using Docker Compose, including
...
# Deploying GDZ.WORKS Server Database Version with Docker Compose
...
<Steps>
  ...
  <Callout type="warning">
    Please log in to GDZ.WORKS using the user account; the administrator account is only for managing Casdoor.
  </Callout>
  ...
  Visit your GDZ.WORKS service at [http://localhost:3210](http://localhost:3210). The account credentials for the application can be found in the report from step `2`.
</Steps>
...
<Steps>
  ...
  You can access your GDZ.WORKS service at `http://your_server_ip:3210`. The account credentials for the application can be found in the report from step `2`.

  <Callout type="warning">
    If your service can accessed via the public network,
    we strongly recommend disabling the registration,
    refer to the [documentation](https://gdz.works/docs/self-hosting/advanced/auth/next-auth/casdoor)
...
<Steps>
  ...
  - Domain setup for the GDZ.WORKS service: `lobe.example.com`
  ...
  You can access your GDZ.WORKS service via `https://lobe.example.com`. The account credentials for the application can be found in the report from step `3`.
...
This section mainly introduces the configurations that need to be modified to customize the deployment of the GDZ.WORKS service in different network environments. Before starting, you can download the [Docker Compose configuration file](https://raw.githubusercontent.com/lobehub/lobe-chat/HEAD/docker-compose/local/docker-compose.yml) and the [environment variable configuration file](https://raw.githubusercontent.com/lobehub/lobe-chat/refs/heads/main/docker-compose/local/.env.example).
...
- GDZ.WORKS requires communication with Casdoor, so you need to configure Casdoor's Issuer.

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

This configuration will affect GDZ.WORKS's login authentication service, and you need to ensure that the URL of the Casdoor service is correct. You can find common manifestations and solutions for errors in this configuration in the [FAQ](#faq).

- Additionally, you need to allow the callback URL in Casdoor to point to the GDZ.WORKS address:
...
- GDZ.WORKS needs to provide a public access URL for object files for the LLM service provider, hence you need to configure MinIO's Endpoint.
...
Cause: GDZ.WORKS cannot access the authentication service.

Solutions:

- Check whether your authentication service is running properly and whether GDZ.WORKS's network can reach the authentication service.

- A straightforward troubleshooting method is to use the `curl` command in the GDZ.WORKS container terminal to access your authentication service at `https://auth.example.com/.well-known/openid-configuration`. If JSON format data is returned, it indicates your authentication service is functioning correctly.

````markdown
## Extended Configuration

To enhance your GDZ.WORKS service, you can perform the following extended configurations according to your needs.
...
- `lobe.example.com`: This is your GDZ.WORKS service domain, which needs to reverse proxy to the GDZ.WORKS service port, default is `3210`.
...
- Name and display name should be `GDZ.WORKS`.
...
<Callout type="warning">
  Please remember to configure the corresponding S3 service provider's CORS settings to ensure that GDZ.WORKS can access the S3 service correctly.
...
3. Restart the GDZ.WORKS service:

   ```sh
   docker compose up -d
   ```

At this point, you have successfully deployed the GDZ.WORKS database version, and you can access your GDZ.WORKS service at `https://lobe.example.com`.
